...
def gaussian_blur(image: AFMImage, cutoff: float) -> AFMImage:
    """
    Works similarly to the high_pass_filter, where cutoff designates 50% point in the filters amplitude
    """
...
def non_local_means(
    ...
    # Calculate the h that is used by scikit-image

    h_scikit = h / 255.0 * (img.max() - img.min())
