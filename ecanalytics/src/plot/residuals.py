...
from ..config import RESIDUAL_PLOT_DEFAULT_FIGSIZE, FIGURE_SETTINGS, SNS_LINEPLOT_DEFAULT_SETTINGS
...
@singledispatch
def residual_distr(data: DataQuality, title: str | None = None, min_bound: float = 1.0, **kwargs) -> PlotResult:
    return residual_distr(data.residuals, title, min_bound, **kwargs)

@residual_distr.register(pd.DataFrame)
def __residual_distr_data(data: pd.DataFrame, title: str | None = None, min_bound: float = 1.0, **kwargs) -> PlotResult:
    ...
    config["marginal_kws"] = {"facecolor": to_rgba(color, 0.5), "edgecolor": color, "stat": "density"} | kwargs.get("marginal_kws", {})
