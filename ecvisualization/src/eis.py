...
from .palette import NEIColorPalette as Palette
from .palette import ColoredObject
from .config import FREQUENCY_TOLERANCE

class SingleExpEIS(ABC):
    RequiredDataSeries = {"Frequency", "Impedance", "Resistance", "Neg. Reactance", "Phase"}
    ...
    def load(cls, filePath: str) -> 'SingleExpEIS':
        ...
        if "Impedance Spectroscopy" not in content:
            raise ValueError(f"File '{filePath}' does not contain Impedance Spectroscopy data.")
        
        deviceGuess[SingleExpEISBioLogic if content.startswith("EC-Lab ASCII FILE") else SingleExpEISPalmSens] += 1
    ...
    def _extractDataField(self) -> str: 
        ...
        for match, group in groupby(enumerate(columnCounts), lambda x: x[1] == maxColumns):
            if match:
