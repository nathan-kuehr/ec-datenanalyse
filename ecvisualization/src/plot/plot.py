...
def __plot_single_eis_data(data: pd.DataFrame, x: str, y: str, title: str | None = None, **kwargs) -> PlotResult:
    config = {
        "data": data,
        "x": x,
        "y": y,
    ...
    if kwargs.get("hue") is None:
        ...
        if len(palettes) > 1 and Npalettes != Nhue:
            raise ValueError(f"Number of palettes ({Npalettes}) does not match number of hue categories ({Nhue}).")
        elif len(palettes) == 1:
...
@plot.register(list)
def __plot_multiple_eis(data: list[EIS|pd.DataFrame], x: str, y: str, title: str | None = None, **kwargs) -> PlotResult:
    if len(data) == 0:
...
def __bode_data(data: pd.DataFrame, title: str | None = None, **kwargs) -> PlotResult:
    ...
    with plt.rc_context(FIGURE_SETTINGS):
        if title is not None:
            fig.suptitle(title, fontsize=FIGURE_SETTINGS["axes.titlesize"], fontweight=FIGURE_SETTINGS["axes.titleweight"], y=0.98)

        plot(data, x="Frequency", ax=ax[0], y="Impedance", title="Magnitude", **kwargsIntermed)
...
@bode.register(list)
def __bode_multiple(data: list[EIS|pd.DataFrame], title: str | None = None, **kwargs) -> PlotResult:
    if len(data) == 0:
        raise ValueError("Data list is empty.")
...
def __fresponse_data(data: pd.DataFrame, y: str, title: str | None = None, **kwargs) -> PlotResult:
    return plot(data, x="Frequency", y=y, title=title, **kwargs)

@fresponse.register(list)
def __fresponse_multiple(data: list[EIS|pd.DataFrame], y: str, title: str | None = None, **kwargs) -> PlotResult:
    if len(data) == 0:
        raise ValueError("Data list is empty.")
